...
def test_load_cached_skips_row_missing_slide_type(tmp_path: Path) -> None:
    t = tmp_path / "a.pptx"
    Presentation().save(str(t))
    sha = template_sha256(t)
    c = tmp_path / ".generated" / "layout" / "a.json"
        ...
        json.dumps(
            {
                ...
                "prompt_version": LAYOUT_DESC_CACHE_PROMPT_VERSION,
                "layouts": [
                    {"index": 0, "name": "A", "description": "no type field"},
                    {
                        "index": 1,
                        "name": "B",
                        "slide_type": "content",
...
def test_load_cached_skips_row_missing_slide_has_image_placeholder(
    tmp_path: Path,
) -> None:
    t = tmp_path / "a.pptx"
    Presentation().save(str(t))
...
def test_save_and_roundtrip(tmp_path: Path) -> None:
    ...
    save_cache(
        ...
        descriptions={
            0: _ld("header", "first"),
            1: _ld("content", "second", slide_has_image_placeholder=True),
        },
    )
    assert c.is_file()
    assert load_cached(c, sha) == {
...
def test_ensure_layout_descriptions_fills_only_missing(
    ...
    g = Mock()
    g.describe_layout.return_value = _ld("content", "new1")
    d = ensure_layout_descriptions(
        generator=g, template_pptx=t, layouts=layouts, generated_dir=gen_dir
    )
    assert d[0] == _ld("header", "cached0")
    assert d[1] == _ld("content", "new1")
    assert g.describe_layout.call_count == 1


def test_default_template_cache_path_and_sha(tmp_path: Path) -> None:
    """When template_pptx=None, cache uses synthetic key + version-derived sha."""
    p = cache_path(None, tmp_path / ".generated")
    assert p.name == "default-pptx.json"
    sha = template_sha256(None)
    assert len(sha) == 64
    # Stable across calls
    assert template_sha256(None) == sha


def test_ensure_layout_descriptions_default_pptx_set(tmp_path: Path) -> None:
    gen_dir = tmp_path / ".generated"
    gen_dir.mkdir()
    layouts = [
        LayoutInfo(index=0, name="Title Slide", placeholders=()),
        LayoutInfo(index=1, name="Title and Content", placeholders=()),
    ]
    g = Mock()
    g.describe_layout.side_effect = [
        _ld("header", "title slide"),
        _ld("content", "content slide"),
    ]
    d = ensure_layout_descriptions(
        generator=g, template_pptx=None, layouts=layouts, generated_dir=gen_dir
    )
    assert d == {
        0: _ld("header", "title slide"),
        1: _ld("content", "content slide"),
    }
    assert (gen_dir / "layout" / "default-pptx.json").is_file()

    # Second call hits cache, no new LLM calls.
    g2 = Mock()
    d2 = ensure_layout_descriptions(
        generator=g2, template_pptx=None, layouts=layouts, generated_dir=gen_dir
    )
    assert d2 == d
    assert g2.describe_layout.call_count == 0
